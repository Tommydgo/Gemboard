...
router.get('/boards', auth, async (req, res) => {
    const infos = await all_board_infos()
...
router.get('/boards/:id', auth, async (req, res) => {
    ...
    const infos = await board_infos(board_id)
...
router.post('/boards', auth, async (req, res) => {
    ...
    const board = await board_infos(board_id)
...
router.put('/boards/:id', auth, async (req, res) => {
    ...
    const status = await update_board(req.body.name, board_id)
    if (status === 0)
        return res.status(404).json({ "msg": "Not found" });
    const updated_board = await board_infos(board_id);
...
router.delete('/boards/:id', auth, async (req, res) => {
    ...
    if (Number.isNaN(board_id))
        return res.status(400).json({ "msg": "Bad parameter" });
    const st : number = await delete_board(board_id)
